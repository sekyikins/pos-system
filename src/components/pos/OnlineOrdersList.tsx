...
import { getOnlineOrders, updateOnlineOrderStatus, startProcessingOnlineOrder, getUsers } from '@/lib/db';
...
export function OnlineOrdersList() {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      const [allOrders, allStaff] = await Promise.all([getOnlineOrders(), getUsers()]);
      setOrders(allOrders);
      setStaff(allStaff);
    } catch {
      addToast('Failed to load online orders', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [addToast]);
  ...
  const handleStartProcessing = async (id: string) => {
    ...
    try {
      ...
      addToast(`Order ${id.slice(-6).toUpperCase()} is now being processed by you`, 'success');
      await load();
  ...
  const handleUpdateStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      addToast(`Order ${id.slice(-6).toUpperCase()} updated to ${nextStatus}`, 'success');
      await load();
    } catch {
  ...
  const activeOrders = useMemo(() => orders.filter(o => o.status === 'PENDING' && !o.processingStaffId), [orders]);
  const processingOrders = useMemo(() => orders.filter(o => (o.status === 'CONFIRMED' || o.status === 'SHIPPED') && o.processingStaffId), [orders]);
  const historyOrders = useMemo(() => orders.filter(o => 
    (o.status === 'DELIVERED' || o.status === 'CANCELLED') && 
    (o.processedBy === user?.id || user?.role === 'ADMIN' || user?.role === 'MANAGER')
  ), [orders, user]);

  const displayOrders = currentTab === 'ACTIVE' ? activeOrders : currentTab === 'PROCESSING' ? processingOrders : historyOrders;
  ...
  const groupedHistory = useMemo(() => {
    ...
    displayOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).forEach(o => {
  ...
  return (
    <div className="p-4 space-y-6">
      {/* Tab Switcher */}
      <div className="flex p-1 bg-muted/30 rounded-2xl w-full max-w-2xl mx-auto">
      ...
      {displayOrders.length === 0 ? (
        ...
        <div className="space-y-8">
          {Object.entries(groupedHistory).map(([group, orders]) => (
               ...
               <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                  ...
                  <p className="text-xs font-medium">
                    {isBeingProcessedByMe ? (
                      ...
                      <span className="text-info">Processing started by {processingStaff?.name || 'Unknown'}</span>
                    )}
                  </p>
               </div>
             )}
